...
async function runAnalysis() {
    ...
    const players = rawPlayers.map((p: any) => ({ ...p, history: rawHistory[p.id] || [] })).filter((p: any) => p.history.length > 0);
    ...
    for (let week = 5; week <= 21; week++) {
        ...
        players.forEach((p: any) => {
            const hUpToNow = p.history.filter((h: any) => h.round <= week).sort((a: any, b: any) => b.round - a.round);
        ...
        weekData.forEach(({ p, blended }: any) => {
            const b = blended;
            ...
            // POSITION SPECIFIC POWER OVERRIDES
            let power = 0.3;
            if (p.element_type === 1 || p.element_type === 2) power = 0.7;

            const reliability = Math.pow(b.minutes / globalMax.min, power);
            let rawScore = 0;
            switch (p.element_type) {
                case 1: rawScore = (0.35 * nCS) + (0.35 * nSaves); break;
                case 2: rawScore = (0.45 * nCS) + (0.30 * nXA) + (0.15 * nXG); break;
