...
const CSV_URL = "https://raw.githubusercontent.com/vaastav/Fantasy-Premier-League/master/data/2024-25/gws/merged_gw.csv";
...
async function ingestHistorical() {
    console.log("Starting historical data ingestion (2024/25)...");
    ...
    const tx = db.transaction((rows: any[]) => {
        for (const row of rows) {
            ...
            // Strategy 3: Handle "Surname Firstname" (e.g. Tomiyasu Takehiro)
            if (!playerId) {
                // Try reversing words? Too expensive for all.
                // Just check specific known failures if needed, but let's see.
            }
            ...
            // Generate pseudo fixture ID for past season: -1 * (SeasonStartYear * 1000 + gameweek * 100 + matchIndex?)
            // Actually, we can just use negative integers.
            // But we need unique ID per match.
            // The CSV has 'fixture' column (integer).
            // We can use -(2024000 + parseInt(row.fixture))

            const fixtureId = -(2024000 + parseInt(row.fixture));

            // Construct data object matching FPL API structure where possible
            const historyData = {
                ...row,
                season_name: '2024/25', // Mark explicitly
                ...
                // Add other numeric parsings if needed
            };
