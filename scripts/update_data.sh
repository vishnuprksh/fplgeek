...
if [[ "$1" == "--docker" ]]; then
    ...
    docker compose -f "$REPO_ROOT/docker-compose.yml" exec server npx tsx ingest.ts

    echo "📊 Step 2: Generating datasets..."
    docker compose -f "$REPO_ROOT/docker-compose.yml" exec server npx tsx scripts/generate_dataset.ts
...
python3 scripts/ai_manager.py

# 4. Generate Predictions
echo "🔮 Step 4: Generating future predictions..."
python3 scripts/ai_manager.py --predict
